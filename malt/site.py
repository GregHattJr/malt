...
def load_site_config():
    ...
    if home() and os.path.isfile(home('site.py')):
        with open(home('site.py'), encoding='utf-8') as file:
            exec(file.read(), config)
    elif home() and os.path.isfile(home('ark.py')):
...
def find_theme(name):
    ...
    if os.getenv('MALT_THEMES'):
        if os.path.isdir(os.path.join(os.getenv('MALT_THEMES'), name)):
            return os.path.join(os.getenv('MALT_THEMES'), name)
